...
export default function DimensionPanel() {
  ...
  const { rotateImage, cropImage } = useImageProcessor({
    onResult: (imageData) => {
      ...
      dispatch({
        ...
        payload: { imageId: activeImage.id, originalData: imageData, width: imageData.width, height: imageData.height },
      });
    },
  });
  ...
  useEffect(() => {
    ...
    return () => { delete window.__hyperionStartCrop; };
  }, [startCrop]);

  // ── Aspect ratio selection ─────────────────────────────────────────────
  const selectRatio = useCallback((ratio) => {
    if (!activeImage) return;
    const cropRect = computeCropForRatio(ratio, activeImage.width, activeImage.height);
    dispatch({
      type: 'SET_CROP',
      payload: {
        imageId: activeImage.id,
  ...
  const applyCrop = useCallback(() => {
    ...
    cropImage(activeImage.originalData, x, y, width, height);
  ...
  const rotate = useCallback((direction) => {
    ...
    dispatch({ type: 'ROTATE_IMAGE', payload: { imageId: activeImage.id, direction } });
  ...
  const applyResize = useCallback(() => {
    ...
    dispatch({ type: 'UPDATE_IMAGE_DATA', payload: { imageId: activeImage.id, originalData: newData, width: w, height: h } });
    push(`Resize to ${w}×${h}`);
    setConfirmResize(false);
  ...
  if (!activeImage) {
    return (
      <div className="flex-1 flex items-center justify-center p-6 text-center">
        <p className="text-gray-600 text-sm">Open an image to use dimension tools</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col overflow-y-auto">
      ...
      <div className="panel-section">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-semibold text-gray-300 uppercase tracking-wider">Crop</span>
          <span className="text-xs text-gray-600">{activeImage.width} × {activeImage.height} px</span>
        </div>
        ...
        {!cropActive ? (
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => dispatch({ type: 'CANCEL_CROP', payload: { imageId: activeImage.id } })}
      ...
      {/* ── Rotate ── */}
      <div className="panel-section">
        <span className="text-xs font-semibold text-gray-300 uppercase tracking-wider block mb-3">Rotate</span>
        <div className="flex gap-2">
          <button
            onClick={() => rotate('ccw')}
          ...
          >
            <span style={{ display: 'inline-block', transform: 'scaleX(-1)' }}>↻</span> 90° CCW
          </button>
          <button
            onClick={() => rotate('cw')}
            className="flex-1 flex items-center justify-center gap-1.5 py-2 text-xs bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white rounded border border-gray-700 transition-colors"
      ...
      {/* ── Resize ── */}
      <div className="panel-section">
        <span className="text-xs font-semibold text-gray-300 uppercase tracking-wider block mb-3">Resize</span>
        <div className="space-y-2">
          ...
          {resizeError && (
            <p className="text-xs text-red-400">{resizeError}</p>
          )}
