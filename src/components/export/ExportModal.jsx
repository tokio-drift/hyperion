...
import { imageDataToBlob, downloadBlob, mimeToExtension, replaceExtension } from '../../utils/exportUtils';
...
export default function ExportModal() {
  ...
  async function getFinalImageData(image, adjustments) {
    ...
    return applyAllAdjustments(image.originalData, adjustments);
  }
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const name = filename || replaceExtension(activeImage.name, mimeToExtension(format));
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          ...
          <button onClick={close} className="text-gray-500 hover:text-white text-xl leading-none transition-colors">×</button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-2 font-medium">Filename</label>
            <input
          ...
          {hasMultiple && (
            <div className="rounded-lg bg-gray-800 border border-gray-700 px-3 py-2.5">
              <p className="text-xs text-gray-400">
                <span className="text-yellow-400 font-medium">{state.images.length} images loaded.</span>
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          {hasMultiple && (
            <button onClick={handleExportAll} disabled={exporting}
              ...
              {batchProgress ? `${batchProgress.done}/${batchProgress.total}` : 'Export All'}
            </button>
          )}
          <button onClick={handleExport} disabled={exporting || !activeImage}
