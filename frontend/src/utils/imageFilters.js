...
export function applyVignette(
  ...
  const rx =
    roundness >= 0
      ? aspectX + (1 - aspectX) * roundness
      : aspectX * (1 + roundness);
  const ry =
    roundness >= 0
      ? aspectY + (1 - aspectY) * roundness
      : aspectY * (1 + roundness);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (isBlack) {
        ...
        data[idx + 1] = clamp(
          data[idx + 1] + (255 - data[idx + 1]) * vigAmount,
        );
        data[idx + 2] = clamp(
          data[idx + 2] + (255 - data[idx + 2]) * vigAmount,
        );
      }
    }
  }
...
export function applyAdjustmentsToBuffer(buffer, width, height, adjustments) {
  const {
    exposure = 0,
    brightness = 0,
    contrast = 0,
...
export function blendMasksIntoBuffer(buffer, width, height, masks) {
  ...
  for (const mask of masks) {
    ...
    applyAdjustmentsToBuffer(scratchBuffer, width, height, mAdj);
    ...
    for (let i = 0; i < buffer.length; i += 4) {
      ...
      if (maskVal === 255) {
        ...
        buffer[i + 1] = Math.round(
          buffer[i + 1] + (scratchBuffer[i + 1] - buffer[i + 1]) * t,
        );
        buffer[i + 2] = Math.round(
          buffer[i + 2] + (scratchBuffer[i + 2] - buffer[i + 2]) * t,
        );
      }
    }
  }
...
export function applyAllAdjustments(originalData, adjustments, masks = []) {
  const buffer = new Uint8ClampedArray(originalData.data);

  applyAdjustmentsToBuffer(
    buffer,
    originalData.width,
    originalData.height,
    adjustments,
  );
  blendMasksIntoBuffer(buffer, originalData.width, originalData.height, masks);
